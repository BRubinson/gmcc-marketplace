...
// BASE_PROJECT promotion — the third sync direction.
//
...
extension Store {
    ...
    public func dopePromote(_ req: DopePromoteRequest) throws -> DopePromoteResponse {
        try dbQueue.write { db in
            ...
            var sources = try self.dopeScopeCandidates(
            ...
            for source in sources {
                ...
                let base = try Row.fetchOne(db, sql: """
                    ...
                    """, arguments: [projectUuid, source.code]).map(Self.dopeScopeRow)
                ...
                if let base {
                    ...
                    let counts = try self.copyDopeTree(db, from: source, into: uuid)
                    promoted.append(DopePromotedScope(
