...
//
// The three scopes are the prompt's own wording: searching from a PROMPT's
// context, from the SESSION, or across the PROJECT. Each resolves to a set of
// dope scopes and the FTS query is identical in every case; only the scope
// filter and the optional mask post-filter change.
//
// --only-masks is a POST-FILTER over resolver provenance, not a SQL join: the
// stack is resolved once and hits are kept when their dot-path came from the
// overlay. That keeps the FTS query the same shape with and without the flag.
...
extension Store {
    ...
    public func dopeSearch(_ req: DopeSearchRequest) throws -> DopeSearchResponse {
        ...
        return try dbQueue.read { db in
            ...
            let scopes = try self.searchScopeRows(db, req: req)
            ...
            for source in DopeSearchSource.allCases {
                arms.append(Self.dopeSearchArm(source, scopePlaceholders: placeholders))
                args.append(pattern)
            ...
            for scope in scopes where scope.tier?.isOverlay == true {
                ...
                let overlayTree = try self.fetchDopeTree(db, scope: scope)
    ...
    private func searchScopeRows(
        _ db: Database, req: DopeSearchRequest
    ) throws -> [DopeScopeRow] {
        switch req.scope {
        case .prompt:
            ...
            return try Row.fetchAll(db, sql: """
                SELECT * FROM dope_scope
                 ...
                 ORDER BY code
                """, arguments: [sessionUuid, promptUuid]).map(Self.dopeScopeRow)
        case .session:
            ...
            return try Row.fetchAll(db, sql: """
                ...
                """, arguments: [sessionUuid]).map(Self.dopeScopeRow)
        case .project:
            ...
            return try Row.fetchAll(db, sql: """
                ...
                """, arguments: [projectUuid]).map(Self.dopeScopeRow)
        }
    }
    ...
    private static func dopeSearchArm(
