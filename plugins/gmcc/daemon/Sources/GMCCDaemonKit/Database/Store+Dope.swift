...
extension Store {
    ...
    public func dopeInit(_ req: DopeInitRequest) throws -> DopeScopeResponse {
        try dbQueue.write { db in try DopeRepository(db: db, store: self).dopeInit(req) }
    }

    public func dopeList(_ req: DopeListRequest) throws -> DopeListResponse {
        try dbQueue.read { db in try DopeRepository(db: db, store: self).dopeList(req) }
    }

    public func dopeGet(_ req: DopeGetRequest) throws -> DopeGetResponse {
        try dbQueue.read { db in try DopeRepository(db: db, store: self).dopeGet(req) }
    }

    public func dopeNodeAdd(_ req: DopeNodeAddRequest) throws -> DopeNodeResponse {
        try dbQueue.write { db in try DopeRepository(db: db, store: self).dopeNodeAdd(req) }
    }

    public func dopeNodeUpdate(_ req: DopeNodeUpdateRequest) throws -> DopeNodeResponse {
        try dbQueue.write { db in try DopeRepository(db: db, store: self).dopeNodeUpdate(req) }
    }

    public func dopeNodeDelete(_ req: DopeNodeDeleteRequest) throws -> DopeNodeDeleteResponse {
        try dbQueue.write { db in try DopeRepository(db: db, store: self).dopeNodeDelete(req) }
    }
    ...
    func fetchDopeScope(_ db: Database, uuid: String) throws -> DopeScopeRow? {
        try DopeRepository(db: db, store: self).fetchDopeScope(uuid: uuid)
    }
    ...
    func bumpScopeRevision(
        ...
        try DopeRepository(db: db, store: self)
    ...
    func instanceRoot(_ db: Database, sessionUuid: String) throws -> String {
        try DopeRepository(db: db, store: self).instanceRoot(sessionUuid: sessionUuid)
    }

    func dopeOwningScope(_ db: Database, level: DopeLevel, nodeUuid: String) throws -> DopeScopeRow {
        try DopeRepository(db: db, store: self).dopeOwningScope(level: level, nodeUuid: nodeUuid)
    }
    ...
    func recordDopeChange(
        ...
        try DopeRepository(db: db, store: self).recordDopeChange(
    ...
    func dopeScopeCandidates(
        ...
        try DopeRepository(db: db, store: self).dopeScopeCandidates(
    ...
    func dopeProjectScopeCandidates(
        ...
        try DopeRepository(db: db, store: self).dopeProjectScopeCandidates(
    ...
    func areaVersions(_ db: Database, scopeUuid: String) throws -> [String: Int64] {
        try DopeRepository(db: db, store: self).areaVersions(scopeUuid: scopeUuid)
    }
    ...
    func wipeDopeTree(_ db: Database, scopeUuid: String) throws {
        try DopeRepository(db: db, store: self).wipeDopeTree(scopeUuid: scopeUuid)
    }
