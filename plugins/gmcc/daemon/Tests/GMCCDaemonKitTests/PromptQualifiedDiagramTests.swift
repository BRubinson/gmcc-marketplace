...
final class PromptQualifiedDiagramTests: XCTestCase {
    ...
    func testSchemaVersionMatchesCompiledConstant() throws {
        ...
        XCTAssertEqual(Migrations.currentSchemaVersion, 23)
        try store.dbQueue.read { db in
            XCTAssertEqual(
                try Int.fetchOne(db, sql: "SELECT MAX(version) FROM schema_migrations"), 23)
            XCTAssertEqual(
                try Int.fetchOne(
                    db, sql: "SELECT COUNT(*) FROM schema_migrations WHERE version = 23"), 1)
