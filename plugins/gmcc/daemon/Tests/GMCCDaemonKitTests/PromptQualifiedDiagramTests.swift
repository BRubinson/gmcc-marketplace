...
final class PromptQualifiedDiagramTests: XCTestCase {
    ...
    func testSchemaVersionMatchesCompiledConstant() throws {
        ...
        XCTAssertEqual(Migrations.currentSchemaVersion, 24)
        try store.dbQueue.read { db in
            XCTAssertEqual(
                try Int.fetchOne(db, sql: "SELECT MAX(version) FROM schema_migrations"), 24)
            XCTAssertEqual(
                try Int.fetchOne(
                    db, sql: "SELECT COUNT(*) FROM schema_migrations WHERE version = 24"), 1)
