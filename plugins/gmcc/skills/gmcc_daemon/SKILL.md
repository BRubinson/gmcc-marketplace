...
| `gm prompt list [--session-uuid U]` | Lightweight stubs (uuid, seq, code, name, status, version); prints `seq. name [status] vN uuid`. |
...
| `gm file-change list [--session-uuid U] [--prompt-uuid] [--path] [--limit]` | Query changes for the current session with ranges joined. |
