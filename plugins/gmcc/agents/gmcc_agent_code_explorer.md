...
4. Produce output that can be consumed by parent workflows (Crack macro, feature-dev, etc.)
...
When invoked with a methodology parameter (for Crack macro compatibility), adapt your exploration:
...
```
gmcc:agent:code_explorer(
  target: "src/auth/",
  goal: "understand authentication flow for adding OAuth",
